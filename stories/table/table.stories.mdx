import { BrowserAnimationsModule } from '@angular/platform-browser/animations';
import { Canvas, Meta, Story } from '@storybook/addon-docs';
import { moduleMetadata } from '@storybook/angular';

import { DATA_SOURCE } from './data';
import { ExpandDemoComponent } from './expand-demo/expand-demo.component';
import { FixedSizeVirtualScrollDemoComponent } from './fixed-size-virtual-scroll-demo/fixed-size-virtual-scroll-demo.component';
import { PlaceholderComponent } from './placeholder/placeholder.component';
import { SortDemoComponent } from './sort-demo/sort-demo.component';
import { StickyColumnsDemoComponent } from './sticky-columns/sticky-columns-demo.component';
import { StickyHeadersDemoComponent } from './sticky-headers.components';

import {
  IconModule,
  ScrollingModule,
  SortModule,
  TableModule,
  ButtonModule,
} from '@alauda/ui';

<Meta
  title="Tables"
  decorators={[
    moduleMetadata({
      imports: [
        BrowserAnimationsModule,
        SortModule,
        IconModule,
        ScrollingModule,
        TableModule,
        ButtonModule,
      ],
      declarations: [
        SortDemoComponent,
        ExpandDemoComponent,
        FixedSizeVirtualScrollDemoComponent,
        StickyColumnsDemoComponent,
        StickyHeadersDemoComponent,
        PlaceholderComponent,
      ],
    }),
  ]}
/>

# Table

Table 组件基于 CdkTable 开发, 加了一些 alauda 自定义的表格样式

## 基础使用

<Canvas>
  <Story
    name="Basic"
    height="600px"
  >
    {{
      template: /* HTML */ `
        <aui-table [dataSource]="dataSource">
          <ng-container auiTableColumnDef="id">
            <aui-table-header-cell *auiTableHeaderCellDef>
              No.
            </aui-table-header-cell>
            <aui-table-cell *auiTableCellDef="let item">
              <div>{{ item.id }}</div>
            </aui-table-cell>
          </ng-container>
          <ng-container auiTableColumnDef="name">
            <aui-table-header-cell *auiTableHeaderCellDef>
              Name
            </aui-table-header-cell>
            <aui-table-cell
              *auiTableCellDef="let item"
              direction="column"
            >
              {{ item.name }}
              <div style="font-size: 12px;color: #96989b;line-height: 16px;">
                {{ item.displayName }}
              </div>
            </aui-table-cell>
          </ng-container>
          <ng-container auiTableColumnDef="value">
            <aui-table-header-cell *auiTableHeaderCellDef>
              Value
            </aui-table-header-cell>
            <aui-table-cell *auiTableCellDef="let item">
              {{ item.value }}
            </aui-table-cell>
          </ng-container>
          <aui-table-header-row
            *auiTableHeaderRowDef="['id', 'name', 'value']; sticky: sticky"
          ></aui-table-header-row>
          <aui-table-row
            *auiTableRowDef="let row; columns: ['id', 'name', 'value'];"
          ></aui-table-row>
        </aui-table>
      `,
      props: {
        dataSource: DATA_SOURCE,
      },
    }}
  </Story>
</Canvas>

## Placeholder 使用

<Canvas>
  <Story
    name="Placeholder"
    height="300px"
  >
    {{ template: '<placeholder-table-demo></placeholder-table-demo>' }}
  </Story>
</Canvas>

## Sort 排序

<Canvas>
  <Story
    name="Sort"
    height="540px"
  >
    {{ template: '<sort-table-demo></sort-table-demo>' }}
  </Story>
</Canvas>

## expand 展开

<Canvas>
  <Story
    name="Expand"
    height="600px"
  >
    {{ template: '<expand-table-demo></expand-table-demo>' }}
  </Story>
</Canvas>

## Header 固定

<Canvas>
  <Story
    name="Sticky Header"
    height="600px"
  >
    {{ template: '<sticky-headers-table-demo></sticky-headers-table-demo>' }}
  </Story>
</Canvas>

## Header 相对父级元素固定

<Canvas>
  <Story
    name="Sticky To Parent"
    height="600px"
  >
    {{
      template: /* HTML */ `
        <div style="height:300px;overflow:auto">
          <h1>Sticky To Parent</h1>
          <aui-table [dataSource]="dataSource">
            <ng-container auiTableColumnDef="id">
              <aui-table-header-cell *auiTableHeaderCellDef>
                No.
              </aui-table-header-cell>
              <aui-table-cell *auiTableCellDef="let item">
                <div>{{ item.id }}</div>
              </aui-table-cell>
            </ng-container>
            <ng-container auiTableColumnDef="name">
              <aui-table-header-cell *auiTableHeaderCellDef>
                Name
              </aui-table-header-cell>
              <aui-table-cell
                *auiTableCellDef="let item"
                direction="column"
              >
                {{ item.name }}
                <div style="font-size: 12px;color: #96989b;line-height: 16px;">
                  {{ item.displayName }}
                </div>
              </aui-table-cell>
            </ng-container>
            <ng-container auiTableColumnDef="value">
              <aui-table-header-cell *auiTableHeaderCellDef>
                Value
              </aui-table-header-cell>
              <aui-table-cell *auiTableCellDef="let item">
                {{ item.value }}
              </aui-table-cell>
            </ng-container>
            <aui-table-header-row
              *auiTableHeaderRowDef="['id', 'name', 'value']; sticky: true"
            ></aui-table-header-row>
            <aui-table-row
              *auiTableRowDef="let row; columns: ['id', 'name', 'value'];"
            ></aui-table-row>
          </aui-table>
        </div>
      `,
      props: {
        dataSource: DATA_SOURCE,
      },
    }}
  </Story>
</Canvas>

## Sticky Columns 固定

<Canvas>
  <Story
    name="Sticky Columns"
    height="500px"
  >
    {{ template: '<sticky-columns-table-demo></sticky-columns-table-demo>' }}
  </Story>
</Canvas>

## aui-table Inputs

| 名称       | 类型  | 默认值 | 描述       |
| ---------- | ----- | ------ | ---------- |
| dataSource | array | -      | table 数据 |

## Directives

| 名称                  | 作用范围         | 描述                                                                                  |
| --------------------- | ---------------- | ------------------------------------------------------------------------------------- |
| auiTableScrollWrapper | aui-table 的父级 | 固定列时候需要在 aui-table 的外层包裹一层，提供左右滚动效果                           |
| auiTableScrollable    | aui-table        | 表格内滚动指令，继承 cdkScrollable 指令，提供上下阴影滚动效果，固定表头时相对表格定位 |
